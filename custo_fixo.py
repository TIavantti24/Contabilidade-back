...
def import_indicadores():
    ...
    try:
        total = import_excel_indicadores(save_path)
        log = ImportLog(tipo="indicadores", filename=filename, total=total, imported_by=user.username)
        db.session.add(log)
        db.session.commit()
        return jsonify({"message": f"{total} indicadores importados.", "total": total})
...
def import_custo():
    ...
    try:
        ...
        log = ImportLog(tipo="custo", filename=filename, total=total, imported_by=user.username)
        db.session.add(log)
        db.session.commit()
...
def auto_detectar():
    ...
    for ind in inds_sorted:
        ...
        for candidato in inds_sorted:
            ...
            if nivel_cand >= nivel_ind:
                continue
            if candidato.area_resultado != ind.area_resultado:
                continue
            nome_cand = (candidato.nome or '').strip()
            nome_ind  = (ind.nome or '').strip()
    ...
    db.session.commit()
    return jsonify({"message": f"{vinculados} indicadores vinculados automaticamente."})